...
class ImageTransferFacade {
  ...
  Future<Uint8List> transfer(Uint8List originData, Uint8List styleData) async {
    ...
    var modelTransferImage = img.copyResize(originImage, width: MODEL_TRANSFER_IMAGE_SIZE, height: MODEL_TRANSFER_IMAGE_SIZE);
    ...
    var modelPredictionImage = img.copyResize(styleImage, width: MODEL_PREDICTION_IMAGE_SIZE, height: MODEL_PREDICTION_IMAGE_SIZE);
    var modelPredictionInput = _imageToByteListUInt8(modelPredictionImage, MODEL_PREDICTION_IMAGE_SIZE, 0 ,255);
    ...
    interpreterPrediction.runForMultipleInputs(inputsForPrediction, outputsForPrediction);

    var inputsForStyleTransfer = [modelTransferInput, styleBottleneck];
    var outputsForStyleTransfer = Map<int, dynamic>();
    var outputImageData = [List.generate(MODEL_TRANSFER_IMAGE_SIZE, (index) =>
        List.generate(MODEL_TRANSFER_IMAGE_SIZE, (index) => List.generate(3, (index) => 0.0))
    )];
